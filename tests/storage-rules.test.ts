...
/** UploadTask is thenable; wrap for assertSucceeds/assertFails Promise typing. */
function upload(task: unknown): Promise<unknown> {
  return Promise.resolve(task as PromiseLike<unknown>);
}
...
/** Wheedle-class A-vs-B: cross-UID upload/read attacks on public and private paths. */
describe("Storage Security Rules (Wheedle-class A-vs-B)", () => {
  ...
  it("allows user A's valid avatar upload and public avatar read", async () => {
    ...
    await assertSucceeds(
      upload(owner.ref(`avatars/${userA.uid}/avatar.jpg`).putString("avatar", "raw", IMAGE_METADATA))
    );
  ...
  it("enforces public image MIME validation for avatars and listing assets", async () => {
    ...
    await assertFails(
      upload(
        owner.ref(`listings/${userA.uid}/payload.pdf`).putString("%PDF", "raw", PDF_METADATA)
      )
    );
  });
  ...
  it("allows verified owners and denies attackers on private KYC assets", async () => {
    ...
    await assertSucceeds(upload(owner.ref(kycPath).putString("id", "raw", IMAGE_METADATA)));
  ...
  it("keeps proof-of-address private and UID-bound", async () => {
    ...
    await assertSucceeds(upload(owner.ref(path).putString("%PDF", "raw", PDF_METADATA)));
    ...
    await assertFails(
      upload(
        attacker
          .ref(`proof_of_address/${userA.uid}/forged.pdf`)
  ...
  it("denies all unrecognized private and public storage prefixes", async () => {
    ...
    await assertFails(
      upload(
        owner
          .ref(`public/${userA.uid}/not-allowlisted.jpg`)
          .putString("nope", "raw", IMAGE_METADATA)
      )
    );
  });
});
