...
import { isFullyVerifiedSeller, profilePhoneVerified } from "../lib/seller-verified";
...
import { parseUsernameForProfileSave, usernameTypingError } from "../lib/username-reservation";
import { getClientCsrfToken } from "../lib/csrf-client";
import { stripClientForbiddenProfileFields } from "../lib/profile-privileged-fields";
...
import { syncProfileDraftToSkyAi } from "../lib/sky-ai-profile-context";
...
/** Account pages stay centered — do not stretch across ultrawide. */
const PROFILE_SHELL = `relative z-10 mx-auto w-full max-w-3xl ${PAGE_PADDING}`;

export type ProfileAccountMode = "overview" | "settings";
...
export default function ProfileAccountClient({ mode }: { mode: ProfileAccountMode }) {
  ...
  const [contentTab, setContentTab] = useState<"listings" | "reviews" | "following">("listings");
...
const tabGroups = [
  ...
  const settingsDraftSnapshot = useMemo(
    () =>
      JSON.stringify({
        ...
        hideOnline, isPublic, showViews, allowFollowers,
        notifEmail, notifMessages, notifAlerts, notifWatchlist, notifOffers,
        notifPriceDrop, notifOffersTrades, notifMessageRequests,
        notifListingActivity, notifListingReplies, notifReactions, notifMentions,
        notifDisputes, notifReports, notifAccountReview, notifPurchases,
        notifRefunds, notifSecurity, notifPlatform, notifIntensity,
        notifQuietHours, notifQuietHoursStart, notifQuietHoursEnd, notifDigest,
      }),
    [
      username, bio, region, discord, instagram, tiktok, website,
      hideOnline, isPublic, showViews, allowFollowers,
  ...
  const settingsDirty = mode === "settings" && settingsBaseline !== null && settingsBaseline !== settingsDraftSnapshot;

  function discardSettingsChanges() {
    if (!settingsBaseline) return;
    const baseline = JSON.parse(settingsBaseline) as Record<string, unknown>;
    setUsername(String(baseline.username ?? ""));
    setBio(String(baseline.bio ?? ""));
    setRegion(String(baseline.region ?? ""));
    setDiscord(String(baseline.discord ?? ""));
    setInstagram(String(baseline.instagram ?? ""));
    setTiktok(String(baseline.tiktok ?? ""));
    setWebsite(String(baseline.website ?? ""));
    setHideOnline(Boolean(baseline.hideOnline));
    setIsPublic(Boolean(baseline.isPublic));
    setShowViews(Boolean(baseline.showViews));
    setAllowFollowers(Boolean(baseline.allowFollowers));
    setNotifEmail(Boolean(baseline.notifEmail));
    setNotifMessages(Boolean(baseline.notifMessages));
    setNotifAlerts(Boolean(baseline.notifAlerts));
    setNotifWatchlist(Boolean(baseline.notifWatchlist));
    setNotifOffers(Boolean(baseline.notifOffers));
    setNotifPriceDrop(Boolean(baseline.notifPriceDrop));
    setNotifOffersTrades(Boolean(baseline.notifOffersTrades));
    setNotifMessageRequests(Boolean(baseline.notifMessageRequests));
    setNotifListingActivity(Boolean(baseline.notifListingActivity));
    setNotifListingReplies(Boolean(baseline.notifListingReplies));
    setNotifReactions(Boolean(baseline.notifReactions));
    setNotifMentions(Boolean(baseline.notifMentions));
    setNotifDisputes(Boolean(baseline.notifDisputes));
    setNotifReports(Boolean(baseline.notifReports));
    setNotifAccountReview(Boolean(baseline.notifAccountReview));
    setNotifPurchases(Boolean(baseline.notifPurchases));
    setNotifRefunds(Boolean(baseline.notifRefunds));
    setNotifSecurity(Boolean(baseline.notifSecurity));
    setNotifPlatform(Boolean(baseline.notifPlatform));
    setNotifIntensity(String(baseline.notifIntensity ?? "balanced"));
    setNotifQuietHours(Boolean(baseline.notifQuietHours));
    setNotifQuietHoursStart(String(baseline.notifQuietHoursStart ?? "22:00"));
    setNotifQuietHoursEnd(String(baseline.notifQuietHoursEnd ?? "08:00"));
    setNotifDigest(Boolean(baseline.notifDigest));
  }
  ...
  const applyProfileData = useCallback((data: ProfileData) => {
    ...
    setSettingsBaseline(JSON.stringify({
      ...
      notifAlerts: data.notifAlerts !== false,
      notifWatchlist: data.notifWatchlist !== false,
      notifOffers: data.notifOffers !== false,
      notifPriceDrop: data.notifPriceDrop || false,
      notifOffersTrades: data.notifOffersTrades !== false,
      notifMessageRequests: data.notifMessageRequests !== false,
      notifListingActivity: data.notifListingActivity !== false,
      notifListingReplies: data.notifListingReplies !== false,
      notifReactions: data.notifReactions !== false,
      notifMentions: data.notifMentions || false,
      notifDisputes: data.notifDisputes !== false,
      notifReports: data.notifReports !== false,
      notifAccountReview: data.notifAccountReview !== false,
      notifPurchases: data.notifPurchases !== false,
      notifRefunds: data.notifRefunds !== false,
      notifSecurity: data.notifSecurity !== false,
      notifPlatform: data.notifPlatform !== false,
      notifIntensity: data.notifIntensity || "balanced",
      notifQuietHours: data.notifQuietHours || false,
      notifQuietHoursStart: data.notifQuietHoursStart || "22:00",
      notifQuietHoursEnd: data.notifQuietHoursEnd || "08:00",
      notifDigest: data.notifDigest || false,
    }));
  ...
  const listingBlockReason = user
    ? getListingBlockReason({
        ...
        profileExists: !!(profile.username || profile.email),
      })
    : null;
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      // Email verification is synced from the ID token by /api/me/profile.
      ...
      setLoading(false);
      showToast("Couldn't load your profile. Please try again.", "error");
    }
  ...
  useEffect(() => {
    ...
    // Legacy /profile?tab=payments → dedicated settings route
    if (mode === "overview" && (tab === "payouts" || tab === "payments" || hash === "settings" || hash === "payment-settings")) {
      const next = tab === "payouts" || tab === "payments" || hash === "payment-settings"
        ? "/profile/settings?tab=payments"
        : hash && hash !== "settings"
    ...
    if (tab === "payouts" || tab === "payments") {
      setActiveTab(isStripeCheckoutVisibleClient() ? "payments" : "account");
    }
    if (!hash) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await claimVerifiedPhoneOnServer(authPhone);
    })().catch(() => {});
    return () => {
      cancelled = true;
    };
  ...
  const tabs = useMemo(() => {
    ...
    return [
      { id: "account", label: "Profile", group: "settings" },
      ...
      { id: "danger", label: "Delete account", group: "settings" },
    ] as const;
  }, []);
  ...
  async function saveProfile(opts?: { bankOnly?: boolean }) {
    ...
    const parsedUsername = parseUsernameForProfileSave(usernameForSave(), { allowSpaces: isAdmin });
    if (!parsedUsername.ok) {
      setUsernameError(parsedUsername.error);
      showToast(parsedUsername.error, "error");
      return;
    }
    const newUsername = parsedUsername.username;
    try {
      ...
      const clientMerge = stripClientForbiddenProfileFields({ ...profilePayload });
      ...
      delete clientMerge.email;
      if (!phoneToSave && isPhoneMarkedVerified) {
        delete clientMerge.phone;
        delete clientMerge.phoneNumber;
      }
      // Server already persisted the profile via Admin SDK. Client merge is
      // limited to non-privileged display fields so rules cannot be bypassed.
      await setDoc(doc(db, "profiles", user.uid), clientMerge, { merge: true }).catch(() => {});
      ...
      setSettingsBaseline(settingsDraftSnapshot);
      flashSaved();
      if (opts?.bankOnly) showToast("Bank details saved.", "success");
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : "Save failed";
      const taken = message === "Username already taken";
      if (taken) setUsernameError("Username already taken");
      setSaving(taken ? "Username taken" : "Save failed");
      showToast(taken ? "Username already taken." : message, "error");
  ...
  async function handleVerifyPhoneCode() {
    ...
    if (result.ok) {
      ...
      if (user) {
        ...
        setPhoneMsg("Could not link phone number. Please sign in again.");
      }
    } else {
  ...
  async function handleRemovePhone() {
    ...
    if (user) {
      try {
        const token = await user.getIdToken();
        const res = await fetch("/api/clear-verified-phone", {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!res.ok) {
          showToast("Could not remove phone number. Try again.", "error");
        }
      } catch {
        showToast("Could not remove phone number. Try again.", "error");
      }
  ...
  const ToggleRow = ({ label, description, val, set }: { label: string; description?: string; val: boolean; set: (v: boolean) => void }) => (
    <label className="flex cursor-pointer items-center justify-between gap-4 py-3.5 text-sm first:pt-0 last:pb-0">
      <span className="min-w-0">
        <span className="block text-[var(--foreground)]">{label}</span>
        {description && <span className="mt-0.5 block text-xs text-[var(--muted)]">{description}</span>}
      </span>
      <span className="relative shrink-0">
        <input type="checkbox" checked={val} onChange={(e) => set(e.target.checked)} className="peer sr-only" />
        <span aria-hidden="true" className="block h-6 w-10 rounded-full bg-[var(--soft-card)] ring-1 ring-[var(--card-border)] transition-colors peer-checked:bg-sky-500 peer-focus-visible:ring-2 peer-focus-visible:ring-sky-500/50">
          <span className="block h-5 w-5 translate-x-0.5 translate-y-0.5 rounded-full bg-white shadow-sm transition-transform peer-checked:translate-x-[1.125rem]" />
        </span>
      </span>
    </label>
  );
  ...
  const usernameFieldError =
    usernameError ||
    usernameTypingError(username, { allowSpaces: !!(user?.email && isAdminEmail(user.email)) });
  ...
  const privacyToggles = [
    { label: "Show online status", description: "Let buyers see when you are active.", val: !hideOnline, set: (v: boolean) => setHideOnline(!v) },
    { label: "Show profile views", description: "Display profile view counts to you.", val: showViews, set: setShowViews },
    { label: "Public profile", description: "Allow your seller profile to appear in the marketplace.", val: isPublic, set: setIsPublic },
    { label: "Allow followers", description: "Let buyers follow your marketplace activity.", val: allowFollowers, set: setAllowFollowers },
  ];
  ...
  useEffect(() => {
    syncProfileDraftToSkyAi(profileDraft);
  }, [profileDraft]);
  ...
  useEffect(() => {
    ...
    if (pending) {
      if (mode === "overview") {
        queueProfileFill(pending);
        router.push("/profile/settings");
        return;
      }
      applyProfileFill(pending);
    }
    const onFill = (e: Event) => {
      const detail = (e as CustomEvent<SkyAiProfileFill>).detail;
      if (!detail) return;
      if (mode === "overview") {
        queueProfileFill(detail);
        router.push("/profile/settings");
        return;
      }
      applyProfileFill(detail);
    };
  ...
  }, [applyProfileFill, mode, router]);

  const settingsSection =
    "rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4 sm:p-5 shadow-[var(--shadow-xs)]";
  const fieldInput =
    "min-h-[44px] w-full rounded-lg border border-[var(--input-border)] bg-[var(--input-bg)] px-3.5 py-2.5 text-sm text-[var(--foreground)] outline-none transition-colors duration-150 placeholder:text-[var(--muted)] focus:border-sky-500/50 focus:ring-2 focus:ring-sky-500/15";
  const primaryBtn =
    "rounded-lg bg-sky-500 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-sky-400 active:scale-[0.98] disabled:opacity-50";
  ...
  if (loading) {
    return (
      <main className="relative min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
        <Background /><Navbar />
        <section className={`${PROFILE_SHELL} pb-10 pt-2 sm:pt-3`}>
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)] transition-colors duration-300">
      ...
      <section className={`${PROFILE_SHELL} ${mode === "settings" ? "max-w-5xl" : ""} pb-10 pt-2 sm:pt-3`}>
        <div className="space-y-5">
          ...
          {mode === "overview" && user?.email &&
            looksAutoGeneratedUsername(contextUsername || username, user.email) && (
              <div className="rounded-xl border border-sky-500/25 bg-sky-500/[0.08] px-4 py-3 text-sm text-sky-800 dark:text-sky-100">
                ...
                <Link href="/profile/settings" className="font-semibold text-sky-700 underline hover:text-sky-900 dark:text-sky-200 dark:hover:text-white">
                  Account settings
                </Link>
                , then save.
              </div>
          ...
          {mode === "settings" ? (
            <>
              <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--card-border)] pb-5">
                <div>
                  <Link href="/profile" className="text-xs font-semibold text-sky-500 hover:text-sky-400">
                    ← Back to profile
                  </Link>
                  ...
                  <p className="mt-1 text-sm text-[var(--muted)]">
                    Manage your profile, account and marketplace preferences.
                  </p>
                </div>
              </div>
          <section id="profile-settings" aria-labelledby="profile-settings-heading" className="space-y-4">
            ...
            <div className="grid gap-5 lg:grid-cols-[190px_minmax(0,1fr)] lg:items-start">
              <nav
                className="grid grid-cols-2 gap-1 border-b border-[var(--card-border)] pb-3 sm:grid-cols-3 lg:sticky lg:top-24 lg:flex lg:border-b-0 lg:pb-0"
                role="tablist"
                aria-label="Settings sections"
              >
                {tabGroups.map((group) => (
                  <div key={group.id} className="contents lg:flex lg:w-full lg:flex-col lg:gap-1">
                    {tabs.filter((tab) => tab.group === group.id).map((tab) => (
                      <button
                        ...
                        className={`min-h-[44px] rounded-lg px-3 py-2.5 text-left text-xs font-semibold transition-colors sm:text-sm lg:w-full ${
                          activeTab === tab.id
                            ? tab.id === "danger"
              ...
              <div className="min-w-0 space-y-4">
                {activeTab === "account" && (
                  <div className="space-y-4">
                    <div className={settingsSection}>
                      <div className="mb-5 flex items-center justify-between gap-4 border-b border-[var(--card-border)] pb-5">
                        <div className="flex min-w-0 items-center gap-3">
                          {avatarUrl ? (
                            <img src={avatarUrl} alt="" className="h-12 w-12 shrink-0 rounded-lg object-cover" />
                          ) : (
                            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-sky-500/10 text-lg font-bold text-sky-500">
                              {initial}
                            </div>
                          )}
                          <div className="min-w-0">
                            <p className="truncate text-sm font-semibold text-[var(--foreground)]">{friendlyName}</p>
                        ...
                        <button
                          ...
                          disabled={!!saving}
                          className="shrink-0 rounded-lg border border-[var(--card-border)] px-3 py-2 text-xs font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--card-hover)] hover:text-[var(--foreground)] disabled:opacity-50"
                        >
                          Change photo
                        </button>
                      ...
                      <div className="mb-5 flex items-start justify-between gap-3">
                        <div>
                          <h3 className="text-base font-bold text-[var(--foreground)]">Public profile</h3>
                      ...
                      <div className="space-y-5">
                        <div>
                          <div className="mb-2 flex items-center justify-between">
                            <label htmlFor="profile-bio-field" className="text-sm font-medium text-[var(--text-secondary)]">Bio</label>
                        ...
                        <div className="grid gap-5 sm:grid-cols-2">
                          <div>
                            <label htmlFor="profile-username-field" className="mb-2 block text-sm font-medium text-[var(--text-secondary)]">Username</label>
                            <div className="relative">
                              ...
                              <input
                                ...
                                aria-describedby={usernameFieldError ? "profile-username-error" : undefined}
                              />
                            </div>
                            {usernameFieldError ? (
                              <p id="profile-username-error" role="alert" className="mt-1.5 text-[11px] text-red-400">
                                {usernameFieldError}
                              </p>
                            ) : (
                          ...
                          <div>
                            <label htmlFor="profile-region-field" className="mb-2 block text-sm font-medium text-[var(--text-secondary)]">Region</label>
                            <select
                              id="profile-region-field"
                              value={region}
                ...
                {activeTab === "security" && (
                  <div className="space-y-4">
                    <div className={settingsSection}>
                      <h3 className="text-base font-bold text-[var(--foreground)]">Password</h3>
                      <p className="mt-1 text-sm text-[var(--muted)]">Use a unique password you do not use elsewhere.</p>
                      <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        <div>
                          <label htmlFor="current-password" className="mb-2 block text-sm font-medium text-[var(--text-secondary)]">Current password</label>
                          <input id="current-password" type="password" autoComplete="current-password" value={pwOld} onChange={(e) => setPwOld(e.target.value)} className={fieldInput} />
                        </div>
                        <div>
                          <label htmlFor="new-password" className="mb-2 block text-sm font-medium text-[var(--text-secondary)]">New password</label>
                          <input id="new-password" type="password" autoComplete="new-password" value={pwNew} onChange={(e) => setPwNew(e.target.value)} className={fieldInput} />
                        </div>
                      </div>
                      <div className="mt-4 flex flex-wrap items-center gap-3">
                        <button type="button" onClick={changePassword} disabled={!pwOld || !pwNew} className="rounded-lg border border-[var(--card-border)] bg-[var(--soft-card)] px-4 py-2.5 text-sm font-semibold text-[var(--foreground)] transition-colors hover:bg-[var(--card-hover)] disabled:opacity-50">
                          Update password
                        </button>
                        {pwMsg && <p className={`text-xs ${pwMsg.includes("updated") ? "text-sky-500" : "text-red-400"}`}>{pwMsg}</p>}
                      </div>
                    </div>
                    <div className={settingsSection}>
                    ...
                    <div className="mb-5 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-0.5 text-sm text-[var(--muted)]">
                          Choose what you hear about from the marketplace.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className={settingsSection}>
                    ...
                    <div className="divide-y divide-[var(--card-border)]">
                      {privacyToggles.map((n) => (
                        <ToggleRow key={n.label} label={n.label} description={n.description} val={n.val} set={n.set} />
                      ))}
                    </div>
                  </div>
                )}
            ...
            {settingsDirty && (
              <div
                role="status"
                className="sticky bottom-3 z-30 mt-5 flex flex-col gap-3 rounded-xl border border-sky-500/20 bg-[var(--elevated)]/95 px-4 py-3 shadow-[var(--shadow-lg)] backdrop-blur sm:flex-row sm:items-center sm:justify-between"
              >
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={!!saving}
                    className="min-h-[44px] flex-1 rounded-lg border border-[var(--card-border)] px-4 py-2 text-sm font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--card-hover)] hover:text-[var(--foreground)] disabled:opacity-50 sm:flex-none"
                  >
                    Cancel
                  </button>
                  <button
                    type="button"
                    onClick={() => saveProfile()}
                    disabled={!!saving || !!usernameFieldError}
                    className={`min-h-[44px] flex-1 px-5 sm:flex-none ${primaryBtn}`}
                  >
          ...
          <header className="border-b border-[var(--card-border)] pb-5">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex min-w-0 items-center gap-3.5">
                {avatarUrl ? (
                  <img src={avatarUrl} alt="" className="h-16 w-16 shrink-0 rounded-xl border border-[var(--card-border)] object-cover sm:h-[4.5rem] sm:w-[4.5rem]" />
                ) : (
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl border border-sky-500/25 bg-sky-500/10 text-xl font-bold text-sky-500 sm:h-[4.5rem] sm:w-[4.5rem] sm:text-2xl">
                    {initial}
                  </div>
                )}
                <div className="min-w-0">
                  <h1 className="truncate text-xl font-bold tracking-tight text-[var(--foreground)] sm:text-2xl">{friendlyName}</h1>
                  <p className="mt-0.5 truncate text-sm text-[var(--muted)]">@{handle}</p>
                </div>
              </div>

              <div className="flex w-full gap-2 sm:w-auto sm:shrink-0">
                <Link href="/profile/settings" className={`inline-flex min-h-[44px] flex-1 items-center justify-center ${primaryBtn} px-4 py-2.5 sm:flex-none`}>
                  Edit profile
                </Link>
                {(contextUsername || username) && (
                  <Link href={sellerProfilePath(contextUsername || username)} className="inline-flex min-h-[44px] flex-1 items-center justify-center rounded-lg border border-[var(--card-border)] bg-[var(--soft-card)] px-4 py-2.5 text-sm font-semibold text-[var(--text-secondary)] transition-colors hover:border-sky-500/25 hover:text-[var(--foreground)] sm:flex-none">
                    View public
                  </Link>
                )}
                <Link href="/profile/settings" aria-label="Open profile settings" title="Profile settings" className="inline-flex min-h-[44px] w-11 shrink-0 items-center justify-center rounded-lg border border-[var(--card-border)] bg-[var(--soft-card)] text-[var(--muted)] transition-colors hover:border-sky-500/25 hover:text-[var(--foreground)]">
                  <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-4 w-4"><path strokeLinecap="round" strokeLinejoin="round" d="M12 15.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Z"/><path strokeLinecap="round" strokeLinejoin="round" d="m19.4 15 .1 1.5-2 1.2-1.2-.8a7.4 7.4 0 0 1-1.4.8l-.3 1.4h-2.3l-.4-1.4a7.4 7.4 0 0 1-1.4-.8L9.3 18l-2-1.2.1-1.5a7.8 7.8 0 0 1-.7-1.4l-1.3-.5V11l1.3-.5c.2-.5.4-1 .7-1.4L7.3 7.6l2-1.2 1.2.8c.4-.3.9-.5 1.4-.7l.4-1.5h2.3l.3 1.5c.5.2 1 .4 1.4.7l1.2-.8 2 1.2-.1 1.5c.3.4.5.9.7 1.4l1.3.5v2.4l-1.3.5c-.2.5-.4 1-.7 1.4Z"/></svg>
                </Link>
              </div>
            </div>
            ...
            <dl className="mt-5 grid grid-cols-3 divide-x divide-[var(--card-border)] border-y border-[var(--card-border)] py-3">
              <div className="min-w-0 px-3 first:pl-0">
                ...
                <dd className="mt-1 flex items-center gap-1 text-sm font-semibold text-[var(--foreground)]">
                  {reviewAvg != null && reviewTotal > 0 ? <><ReviewStars rating={reviewAvg} size="sm" /><span>{reviewTotal}</span></> : "0"}
                </dd>
              </div>
          ...
          >
            {([
              { id: "listings" as const, label: "Listings", count: listingsLoading ? null : activeListings.length },
              { id: "reviews" as const, label: "Reviews", count: reviewsLoading ? null : profileReviews.length },
              { id: "following" as const, label: "Following", count: followingLoading ? null : followingList.length },
            ]).map((tab) => (
              <button
                key={tab.id}
                type="button"
                ...
                className={`min-h-[44px] px-3.5 py-2.5 text-sm font-semibold transition-colors ${
                  contentTab === tab.id
                    ? "border-b-2 border-sky-500 text-[var(--foreground)]"
                    : "border-b-2 border-transparent text-[var(--muted)] hover:text-[var(--foreground)]"
                }`}
              >
                {tab.label}
                {tab.count != null ? (
                  <span className="ml-1.5 text-xs font-medium text-[var(--muted)]">{tab.count}</span>
                ) : null}
              </button>
            ))}
          </nav>
            ...
            <section id="profile-listings-panel" role="tabpanel" aria-labelledby="profile-listings-tab" className="space-y-3">
              <div className="flex items-end justify-between gap-3">
                ...
                {!listingsLoading && activeListings.length > 0 ? (
                  <Link href="/post/ai" className="ml-auto inline-flex min-h-[40px] items-center rounded-lg bg-sky-500 px-3.5 text-xs font-semibold text-white transition-colors hover:bg-sky-400">
                    New listing
                  </Link>
                ) : null}
              </div>
              ...
              {listingsLoading ? (
                ...
                <EmptyState
                  className="border-0 bg-transparent px-4 py-8 sm:py-10"
          ...
          {contentTab === "reviews" && (
            <section id="profile-reviews-panel" role="tabpanel" aria-labelledby="profile-reviews-tab" className="space-y-3">
              ...
              {reviewsLoading ? (
                ...
                <EmptyState className="border-0 bg-transparent px-4 py-8 sm:py-10" title="No reviews yet" description="Reviews from completed trades will appear here." />
              ) : (
          ...
          {contentTab === "following" && (
            <section id="profile-following-panel" role="tabpanel" aria-labelledby="profile-following-tab" className="space-y-3">
              ...
              {followingLoading && followingList.length === 0 ? (
                ...
                <EmptyState className="border-0 bg-transparent px-4 py-8 sm:py-10" title="Not following anyone yet" description="Follow sellers to keep up with their latest listings." actionLabel="Browse marketplace" actionHref="/search" />
              ) : (
                <ul className="divide-y divide-[var(--card-border)] border-y border-[var(--card-border)]">
                  {followingList.map((f) => {
                    const followInitial = (f.username !== "Seller" ? f.username : f.sellerId).charAt(0).toUpperCase();
                    return (
                      <li key={f.sellerId}>
                        <Link href={f.href} className="flex min-h-[56px] items-center gap-3 py-2 text-sm font-semibold text-[var(--foreground)] transition-colors hover:text-sky-500">
                          {f.photoURL ? (
                            <img src={f.photoURL} alt="" className="h-9 w-9 rounded-lg border border-[var(--card-border)] object-cover" />
                          ) : (
                            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--soft-card)] text-xs font-bold text-sky-500">{followInitial}</span>
                          )}
