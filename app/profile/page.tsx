...
const tabGroups = [
  ...
  const ToggleRow = ({ label, val, set }: { label: string; val: boolean; set: (v: boolean) => void }) => (
    <label className="flex cursor-pointer items-center justify-between gap-4 py-2.5 text-sm first:pt-0 last:pb-0 hover:bg-white/[0.02]">
      ...
      <input type="checkbox" checked={val} onChange={(e) => set(e.target.checked)}
        className="h-4 w-4 shrink-0 rounded border-white/[0.06] bg-[var(--soft-card)] text-sky-500 focus:ring-sky-500/30" />
    </label>
  );
  ...
  const statItems = [
    { label: "Sales", value: String(stats.sales), icon: "💰", accent: "from-sky-500 to-sky-400" },
    { label: "Listings", value: String(activeListings.length), icon: "📦", accent: "from-sky-500 to-sky-400" },
        ...
        <div className="space-y-5">
          ...
          {user?.email &&
            looksAutoGeneratedUsername(contextUsername || username, user.email) && (
              <div className="rounded-xl border border-sky-500/25 bg-sky-500/[0.08] px-4 py-3 text-sm text-sky-100">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input id="profile-username-field" type="text" value={username} onChange={(e) => setUsername(e.target.value)}
