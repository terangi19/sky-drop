...
export default function JobsPage() {
  ...
  useEffect(() => {
    const q = query(collection(db, "listings"), where("type", "==", "job"));
      ...
      <section className={`${PAGE_SHELL_WIDE} py-12`}>
        <div className="mb-10 relative overflow-hidden rounded-3xl border border-white/[0.04] bg-gradient-to-b from-white/[0.04] via-transparent to-transparent p-8 sm:p-10 text-center">
          ...
          <div className="relative flex flex-col items-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/15 bg-sky-500/5 px-3.5 py-1 text-[10px] font-semibold text-sky-400 mb-4 tracking-wide uppercase">Job Listings</div>
            <h1 className="text-4xl font-black tracking-tight sm:text-5xl">
              <span className="bg-gradient-to-r from-sky-400 to-sky-400 bg-clip-text text-transparent">Jobs</span>
            </h1>
        ...
        {filtered.length === 0 ? (
          ...
          <div className={LISTING_GRID}>
            {filtered.map((item) => (
              <div key={item.id} className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/[0.04] bg-white/[0.02] transition-all duration-300 hover:bg-white/[0.04] hover:border-sky-500/30 hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(34,211,238,0.15)]">
                ...
                <Link href={`/post/listing/${item.id}`} className="flex flex-1 flex-col p-5">
