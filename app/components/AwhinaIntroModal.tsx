...
export default function AwhinaIntroModal({ open, onGetStarted, onDismiss }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="awhina-intro-title"
      onClick={() => onDismiss(neverAgain)}
    >
      <div
        className={`relative w-full max-w-md max-h-[min(90vh,680px)] overflow-y-auto overflow-x-hidden rounded-3xl border border-white/[0.08] bg-[#0a0c12]/95 shadow-[0_24px_80px_rgba(0,0,0,0.55),0_0_60px_rgba(14,165,233,0.14)] transition-all duration-500 ease-out ${
          visible ? "translate-y-0 scale-100 opacity-100" : "translate-y-4 scale-[0.97] opacity-0"
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-sky-500/[0.14] via-sky-400/[0.06] to-transparent" />

        <div className="relative px-8 pb-8 pt-10 sm:px-10 sm:pb-10 sm:pt-12">
          <div className="mx-auto mb-8 flex h-16 w-16 items-center justify-center rounded-2xl border border-sky-400/25 bg-gradient-to-br from-sky-500/25 to-sky-400/10 shadow-[0_0_32px_rgba(14,165,233,0.25)]">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-sky-400 to-sky-500 text-lg font-bold text-white shadow-[0_0_12px_rgba(14,165,233,0.35)]">
              ✦
            </span>
          </div>

          <div className="space-y-4 text-center">
            <h2 id="awhina-intro-title" className="text-2xl font-black tracking-tight text-white">
              {AWHINA_NAME}
            </h2>
          ...
          <div className="mt-8 space-y-3">
            <button
              ...
              className="w-full rounded-2xl bg-gradient-to-r from-sky-500 to-sky-400 py-3.5 text-sm font-bold text-white shadow-[0_8px_32px_rgba(14,165,233,0.35)] transition-all duration-200 hover:brightness-110 active:scale-[0.98]"
            >
              Get Started
            </button>
            <button
          ...
          <label className="mt-8 flex cursor-pointer items-center justify-center gap-2.5 text-sm text-zinc-500 transition-colors hover:text-zinc-400">
            <input
              ...
              className="h-4 w-4 rounded border-white/20 bg-white/5 text-sky-500 focus:ring-sky-500/40 focus:ring-offset-0"
            />
