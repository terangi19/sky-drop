      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[9999] border-t border-white/[0.04] backdrop-blur-xl md:hidden overflow-x-auto scrollbar-none snap-x snap-mandatory" style={{ backgroundColor: "var(--nav-bg)" }}>
          <div className="flex items-center py-1.5 px-1.5 gap-0.5" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 4px) + 4px)" }}>
            {[
              ...
              >
                {item.badge === "msg" && msgCount > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-sky-500 px-0.5 text-[7px] font-bold text-white shadow-lg shadow-sky-500/30">
                    {msgCount > 9 ? "9+" : msgCount}
                  </span>
                )}
            ...
            >
              ...
              <span className="text-[8px] font-semibold whitespace-nowrap">Logout</span>
            </button>
          </div>
        </nav>
        {/* Scroll hint gradient */}
        <div className="fixed bottom-0 right-0 z-[9999] w-12 h-16 bg-gradient-to-l from-[var(--nav-bg)] to-transparent pointer-events-none md:hidden" />
