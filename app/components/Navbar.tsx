  ...
  return (
    <header className="relative sticky top-0 z-[9999] border-b border-white/10 backdrop-blur-xl" style={{ backgroundColor: "var(--nav-bg)" }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[9999] border-t border-white/[0.04] backdrop-blur-xl md:hidden" style={{ backgroundColor: "var(--nav-bg)" }}>
          <div className="flex items-center py-1.5 pr-12" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 4px) + 4px)" }}>
            <div className="flex items-center gap-0.5 overflow-x-auto scrollbar-none snap-x snap-mandatory pl-1.5">
              {[
                ...
                >
                  {item.badge === "msg" && msgCount > 0 && (
                    <span className="absolute -right-0.5 -top-0.5 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-sky-500 px-0.5 text-[7px] font-bold text-white shadow-lg shadow-sky-500/30">{msgCount > 9 ? "9+" : msgCount}</span>
                  )}
          ...
          <button onClick={handleLogout}
            className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center justify-center h-8 w-8 rounded-lg text-red-400/70 active:bg-red-500/10 transition"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
