  ...
  return (
    <header className="relative sticky top-0 z-[9999] border-b border-white/10 backdrop-blur-xl" style={{ backgroundColor: "var(--nav-bg)" }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[9999] border-t border-white/[0.04] backdrop-blur-xl md:hidden" style={{ backgroundColor: "var(--nav-bg)" }}>
          {/* Row 2: Secondary items — scrollable */}
          <div className="flex items-center gap-1 overflow-x-auto scrollbar-none px-2 py-1 border-b border-white/[0.03]">
            {[
              ...
              <Link key={item.href} href={item.href}
                className={`shrink-0 rounded-lg px-3 py-1 text-[10px] font-semibold transition active:scale-95 ${
                  pathname === item.href ? "text-sky-400 bg-sky-500/10" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                {item.label}
              </Link>
          ...
          {/* Row 1: Main items — always visible */}
          <div className="flex items-center justify-around py-0.5" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 4px) + 4px)" }}>
            {[
              ...
              item.action === "logout" ? (
                <button key="logout" onClick={handleLogout}
                  className="flex flex-col items-center gap-px px-2 py-1 rounded-xl transition active:scale-95 text-red-400/70"
                >
                ...
                <Link key={item.href} href={item.href}
                  className={`relative flex flex-col items-center gap-px px-2 py-1 rounded-xl transition active:scale-95 ${
                    pathname === item.href ? "text-sky-400" : "text-[var(--muted)]"
                  }`}
                >
                  {item.badge === "msg" && msgCount > 0 && (
                    <span className="absolute -right-0.5 -top-0.5 flex h-3 min-w-[12px] items-center justify-center rounded-full bg-sky-500 px-0.5 text-[6px] font-bold text-white">{msgCount > 9 ? "9+" : msgCount}</span>
                  )}
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                  </svg>
      ...
      <style jsx global>{`@keyframes slideUp { from { transform: translateY(12px); opacity: 0; } to { transform: translateY(0); opacity: 1; } } ${user ? "main { padding-bottom: calc(96px + env(safe-area-inset-bottom, 0px)); } @media (min-width: 768px) { main { padding-bottom: 0; } }" : ""}`}</style>
