...
export default function Navbar() {
  ...
  return (
    <header className="relative sticky top-0 z-[9999] border-b border-white/10 backdrop-blur-xl" style={{ backgroundColor: "var(--nav-bg)" }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[9999] border-t border-white/[0.04] backdrop-blur-xl md:hidden overflow-x-auto scrollbar-none" style={{ backgroundColor: "var(--nav-bg)" }}>
          <div className="flex items-center py-1 px-1" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 4px) + 4px)" }}>
            {[
              ...
              { href: "/list-list", label: "My Listings", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
              ...
              <Link key={item.href} href={item.href}
                className={`relative flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl shrink-0 transition active:scale-95 ${
                  pathname === item.href ? "text-sky-400" : "text-[var(--muted)]"
                }`}
              >
                {item.badge === "msg" && msgCount > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-sky-500 px-1 text-[8px] font-bold text-white shadow-lg shadow-sky-500/30">
                    {msgCount > 9 ? "9+" : msgCount}
                  </span>
                )}
