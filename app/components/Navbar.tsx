            ...
            {[
              ...
              item.action === "logout" ? (
                <button key="logout" onClick={handleLogout}
                  className="flex flex-col items-center gap-px px-2 py-1.5 rounded-xl transition active:scale-95 text-red-400/70"
                >
                ...
                <Link key={item.href} href={item.href}
                  className={`relative flex flex-col items-center gap-px px-2 py-1.5 rounded-xl transition active:scale-95 ${
                    pathname === item.href ? "text-sky-400" : "text-[var(--muted)]"
                  }`}
                >
                  {item.badge === "msg" && msgCount > 0 && (
                    <span className="absolute -right-1 -top-0.5 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-sky-500 px-1 text-[7px] font-bold text-white">{msgCount > 9 ? "9+" : msgCount}</span>
                  )}
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                  </svg>
