...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    const q = query(collection(db, "listings"), where("type", "==", "vehicle"));
    const unsub = onSnapshot(q, (snap) => {
      ...
      setListings(items);
    }, (err) => { console.error("Failed to load vehicles:", err); });
    return () => unsub();
  }, []);
  ...
  const vehicleRecentlyViewed = useMemo(() => {
    const vehicleIds = new Set(listings.map((l) => l.id));
    return recentlyViewed.filter((r: any) => {
      if (r.type === "vehicle") return true;
      if (r.type && r.type !== "vehicle") return false;
      return vehicleIds.has(r.id);
    });
  ...
  }, [listings, selectedRegion, selectedCity, searchQuery]);
      ...
      <section className="relative z-10 mx-auto max-w-[1800px] px-4 pb-10 pt-6">
        <div className="mb-10 relative overflow-hidden rounded-3xl border border-white/[0.04] bg-gradient-to-b from-white/[0.04] via-transparent to-transparent p-8 sm:p-10 text-center">
          ...
          <div className="relative flex flex-col items-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-yellow-500/15 bg-yellow-500/5 px-3.5 py-1 text-[10px] font-semibold text-yellow-400 mb-4 tracking-wide uppercase">Vehicles</div>
          ...
          <div className="group relative mt-5 w-full max-w-2xl">
            <div className="absolute -inset-1 rounded-xl bg-gradient-to-r from-yellow-500/30 via-amber-500/30 to-yellow-500/30 opacity-0 blur-lg transition duration-500 group-focus-within:opacity-100" />
            <div className="relative flex items-center rounded-xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-sm transition-all duration-300 focus-within:border-yellow-500/40 focus-within:ring-2 focus-within:ring-yellow-500/25">
              <div className="ml-4 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-yellow-500/10">
              ...
              <input
                ...
                placeholder="Search make, model, title, location..."
              ...
              <Link
                href="/post/ai?type=vehicle"
                className="mr-1.5 ml-1 flex shrink-0 items-center gap-1.5 rounded-lg bg-gradient-to-r from-yellow-500 to-amber-500 px-3 py-2 text-[13px] font-bold text-white shadow-md shadow-yellow-500/20 transition-all duration-200 hover:brightness-110 active:scale-[0.97] sm:gap-2 sm:px-4"
              >
                ...
                <span className="hidden sm:inline">List a Vehicle</span>
                <span className="sm:hidden">List</span>
              </Link>
            </div>
          </div>

          <details className="group mt-4 w-full max-w-2xl overflow-hidden rounded-xl border border-yellow-500/10 bg-gradient-to-b from-yellow-500/[0.03] to-transparent text-left">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2.5 text-[10px] font-bold uppercase tracking-[0.1em] text-yellow-400 transition hover:bg-yellow-500/[0.04] [&::-webkit-details-marker]:hidden">
              <span>📖 How It Works</span>
              <svg
                className="h-3.5 w-3.5 shrink-0 text-yellow-400/70 transition group-open:rotate-180"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
            ...
            <div className="border-t border-yellow-500/10 px-3 pb-3 pt-0.5">
              <div className="grid gap-2.5 sm:grid-cols-2">
                <div className="flex gap-2">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-yellow-500/10 text-[11px]">🔍</span>
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-[var(--foreground)]">Browse Vehicles</p>
                    <p className="mt-0.5 text-[10px] leading-snug text-zinc-500">Find cars, trucks, and more across NZ.</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-yellow-500/10 text-[11px]">💰</span>
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-[var(--foreground)]">Buy or Bid</p>
                    <p className="mt-0.5 text-[10px] leading-snug text-zinc-500">Buy instantly or bid on auctions.</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-yellow-500/10 text-[11px]">💬</span>
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-[var(--foreground)]">Message Seller</p>
                    <p className="mt-0.5 text-[10px] leading-snug text-zinc-500">Ask questions and arrange inspections in chat.</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-yellow-500/10 text-[11px]">✅</span>
                  <div className="min-w-0">
                    <p className="text-[11px] font-bold text-[var(--foreground)]">Complete Purchase</p>
                    <p className="mt-0.5 text-[10px] leading-snug text-zinc-500">Pay securely through Stripe with buyer protection.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-3 w-full max-w-2xl rounded-xl border border-red-500/10 bg-red-500/[0.03] px-3 py-2.5 text-left">
            <p className="text-[10px] leading-relaxed text-red-400/80">
              ⚠️ <span className="font-bold text-red-400">Stay safe.</span> Keep all communication in our chat. Report suspicious behaviour immediately.
            </p>
          </div>
          </div>
        </div>
          ...
          <div className="mb-8">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-[11px] text-zinc-500">
                {filteredListings.length} vehicle{filteredListings.length !== 1 ? "s" : ""}
              ...
              {hotItems.map((item: any) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      borderColor: `rgba(234, 179, 8, ${0.12 + hotGlow * 0.5})`,
                      boxShadow: `0 0 ${Math.round(8 + hotGlow * 44)}px rgba(234, 179, 8, ${0.08 + hotGlow * 0.42})`,
                    }}
                  >
                    <div className="relative overflow-hidden rounded-lg">
                      {item.images?.[0] || item.imageUrl || item.image ? (
                        ...
                        <div className="flex h-28 items-center justify-center rounded-lg bg-gradient-to-br from-yellow-500/10 via-amber-500/10 to-yellow-600/10 text-xs text-zinc-500 sm:h-32">
                          🚗
                        </div>
                      )}
                      <div className="absolute top-1.5 left-1.5">
                        <span className="rounded-full bg-yellow-500/90 px-2 py-0.5 text-[8px] font-bold text-white backdrop-blur-sm">
                          🔥 Trending
                        </span>
                      </div>
                      <div
                        ...
                        style={{
                          borderColor: `rgba(234, 179, 8, ${0.3 + hotGlow * 0.7})`,
                          backgroundColor: `rgba(0, 0, 0, ${0.5 + hotGlow * 0.15})`,
                          color: `rgba(254, 240, 138, ${0.8 + hotGlow * 0.2})`,
                          boxShadow: `0 0 ${Math.round(4 + hotGlow * 18)}px rgba(234, 179, 8, ${0.25 + hotGlow * 0.55})`,
                        }}
                      >
                    ...
                    <div className="mt-2 flex items-start justify-between gap-2">
                      ...
                      <p className="shrink-0 text-sm font-black text-yellow-400">${item.price}</p>
                    </div>
                    {item.location && (
                      <p className="mt-0.5 truncate text-[10px] text-zinc-500">📍 {item.location}</p>
                    )}
                    <div className="mt-1 flex items-center gap-2 text-[10px] text-zinc-500">
                      ...
                      <span
                        ...
                        style={{
                          color: `rgba(254, 240, 138, ${0.55 + hotGlow * 0.45})`,
                          textShadow:
                            hotGlow > 0.15
                              ? `0 0 ${Math.round(4 + hotGlow * 8)}px rgba(250, 204, 21, ${hotGlow * 0.45})`
                              : undefined,
                        }}
                      >
                        ⭐ {hotSaves.toLocaleString()} {hotSaves === 1 ? "save" : "saves"}
            ...
            <h2 className="text-2xl font-black tracking-tight text-white">
              {searchQuery.trim()
                ...
                : selectedCity !== "All"
                  ...
                  : `No vehicles in ${selectedRegion}`}
            </h2>
            <p className="mt-2 text-sm text-zinc-500">
              {searchQuery.trim()
                ? "Try different keywords, another city, or browse all vehicles."
                : "Try another city or region, or list your vehicle in this area."}
            </p>
            <button
              type="button"
              onClick={clearFilters}
