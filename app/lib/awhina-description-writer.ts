...
export function sellerEvidenceCount(facts: DescriptionWriterFacts): number {
...
export async function runAwhinaListingDescriptionWriter(
  ...
  // Billed OpenAI lives in awhina-description-writer.server.ts so Client Components
  // can import validation/facts without pulling firebase-admin into the bundle.
  if (!opts?.generateRawOutput) {
    const attempt: DescriptionWriterAttempt = {
      writer_called: false,
      writer_input: facts,
      writer_validation_result: "not_run",
  ...
  try {
    const raw = await opts.generateRawOutput(facts);
    if (!raw) {
      const attempt: DescriptionWriterAttempt = {
        writer_called: true,
        writer_input: facts,
