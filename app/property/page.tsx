      ...
      <section className={`${PAGE_SHELL_WIDE} py-12`}>
        <div className="mb-10 relative overflow-hidden rounded-3xl border border-white/[0.04] bg-gradient-to-b from-white/[0.04] via-transparent to-transparent p-8 sm:p-10 text-center">
          ...
          <div className="relative flex flex-col items-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/15 bg-sky-500/5 px-3.5 py-1 text-[10px] font-semibold text-sky-400 mb-4 tracking-wide uppercase">Property</div>
            <h1 className="text-4xl font-black tracking-tight sm:text-5xl">
              <span className="bg-gradient-to-r from-sky-400 to-sky-400 bg-clip-text text-transparent">Property</span>
            </h1>
          <BrowseAwhinaAssistantPanel />
          <Link href="/post/ai?type=property" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-sky-500 to-sky-500 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-sky-500/20 transition-all duration-200 hover:shadow-xl hover:shadow-sky-500/30 hover:scale-105 active:scale-95">
        ...
        {listings.length === 0 ? (
          ...
          <div className={LISTING_GRID}>
            {listings.map((item) => (
              <Link key={item.id} href={`/post/listing/${item.id}`} className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/[0.04] bg-white/[0.02] transition-all duration-300 hover:bg-white/[0.04] hover:border-sky-500/30 hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(244,63,94,0.15)]">
                <div className="relative aspect-[4/3] shrink-0 overflow-hidden bg-gradient-to-br from-sky-900/20 to-sky-900/20">
                  ...
                  <div className="absolute top-3 left-3 rounded-full border border-sky-500/20 bg-sky-500/10 px-2 py-0.5 text-[9px] font-bold text-sky-400 backdrop-blur-sm">Property</div>
                </div>
                ...
                <div className="flex flex-1 flex-col p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 text-lg font-black text-sky-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.3)]">${item.price}</span>
                  </div>
